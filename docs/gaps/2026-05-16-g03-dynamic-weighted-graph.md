...
- Verification on 2026-05-17, superseding earlier counts:
  - `PYTHONPATH=packages/core/src:packages/core/tests python3 -m unittest discover -s packages/core/tests`: 139 passed, 1 sqlite-vec skip.
...
- 2026-05-17 current live graph rebuild: `graph_rebuild` job 42 rebuilt 10,108 deterministic `clang_ast` edges from 1,225 files. Edge counts: reads 2,536; writes 4,955; sets_field 995; maps_base 1,616; field_shift 6.
- 2026-05-17 current Stage 2 overlay: real local Ollama `gemma4:e4b` jobs 43 and 44 persisted 11 raw semantic edges; real `doc-nodes-batch` job 45 persisted 6 `doc_box` nodes and 11 doc semantic edges. `global_graph(limit=1500)` now reports 1,123 nodes and 1,500 edges with node kinds `function`, `register`, `doc_box`, and `doc_section`, plus 15 visible semantic edges.
