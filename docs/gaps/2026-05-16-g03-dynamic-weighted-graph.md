...
- Current deterministic-code limitation: the Stage 1 extractor is a pragmatic clang command plus source-span/resolver pipeline with compile_commands-driven preprocessing support. It is not yet full libclang cursor traversal or full Linux kernel compile-database coverage across every include/define edge case.
...
- Verification on 2026-05-17, superseding earlier counts:
  - `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=packages/core/src:packages/core/tests:. python3 -m unittest discover -s packages/core/tests -p 'test_*.py' -v`: 150 passed, 1 sqlite-vec skip.
...
The graph API is live-data backed and now supports a no-seed global graph plus explicit query-scoped and batch semantic-edge actions. The storage contract separates Stage 1 deterministic, evidence overlay, and Stage 2 semantic edges, and the real AMD rebuild now proves Stage 1 function/register operation edges with fields, wrappers, and source paths folded into BoxMatrix-style `in`/`out`/`attr` payloads.

The entity-contract gap is no longer blocking for the current product graph: field, macro/wrapper, and source-file nodes are filtered or folded, edge relations are enum-normalized, and the UI refuses legacy `field/doc/pdf/code` graph-node kinds. Remaining follow-up risk is narrower: the Stage 1 extractor is still pragmatic clang/text-span extraction rather than full libclang cursor traversal across every Linux kernel compile-database edge case, and Stage 2 larger semantic batches still need local-model JSON robustness testing.
