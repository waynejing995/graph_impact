...
- Final automated rerun after the clean-final Stage 2, macro-node, and G06/G07/G15 continuation fixes: core unittest 220 OK with 2 optional sqlite-vec skips; API/MCP unittest 47 OK with 1 optional MCP runtime skip under system Python 3.9; bundled-Python real MCP runtime suite 29 OK with 0 skips; `pnpm --filter web run lint` passed; `pnpm --filter web exec tsc --noEmit` passed; Web API+smoke Playwright 75 passed; visual route Playwright 15 passed; combined Web API+smoke+visual Playwright 90 passed; `git diff --check` passed.
