...
- Latest automated verification after the query-graph performance pass: core unittest 198 OK with 2 optional sqlite-vec skips; API/MCP unittest 45 OK with 1 optional MCP runtime skip; Web API+smoke Playwright 73 passed; visual route Playwright 15 passed; `pnpm --filter web run lint` passed; `pnpm --filter web exec tsc --noEmit` passed; `git diff --check` passed.
...
- Final automated rerun after the clean-final Stage 2 and macro-node fix: core unittest 202 OK with 2 optional sqlite-vec skips; API/MCP unittest 45 OK with 1 optional MCP runtime skip; `pnpm --filter web exec tsc --noEmit` passed; `pnpm --filter web run lint` passed; Web API+smoke Playwright 73 passed; visual route Playwright 15 passed; `git diff --check` passed.
...
The final acceptance run must use a clean named SQLite database. The default `data/asip.db` has now been reset to the clean-final DB and is byte-identical to `/tmp/asip-clean-amd-gemma4-final-current-2026-05-18.db`; the previous dirty local dev DB was backed up to `/tmp/asip-dirty-dev-before-final-default-2026-05-18.db`. Future local browser/API runs may dirty the default DB again, so the named clean-final artifact remains the stable final QA reference.
