...
The current default DB now has full local provider embedding coverage, and a separate quality-evaluation artifact confirms AQ01-AQ09 surface consistency plus explicit provider-vector participation where the current acceptance set exercises it. Provider status must still not imply that a hosted OpenAI-compatible provider generated every current default-DB vector unless job metadata proves it for that DB. Semantic ranking quality at production scale still depends on future corpus distributions and model quality.
