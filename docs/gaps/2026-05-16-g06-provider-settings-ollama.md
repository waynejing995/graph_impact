...
Status: Current default-DB provider gate, local OpenAI-compatible live smoke, batch/query semantic-edge, query-time provider rerank wiring, and full provider embedding coverage are verified; hosted credentialed OpenAI-compatible QA remains unsupplied
...
Local Ollama availability, a small job-level AQ09 provider path, isolated Web BFF AQ09 provenance, Settings UI AQ09 wiring, one real UI-to-BFF isolated-DB AQ09 run, callable semantic-edge product jobs, clean DB local Ollama AQ09 provider checks, and a local OpenAI-compatible live `/v1` smoke are now proven. Hosted credentialed OpenAI-compatible switching still needs credentials or an explicit user-accepted local-compatible endpoint boundary.
...
OpenAI-compatible smoke now validates request shape, safe secret/env header expansion, and a live local `/v1` compatible endpoint. It still does not perform a hosted credentialed OpenAI-compatible endpoint check because no live credentialed endpoint has been supplied.
