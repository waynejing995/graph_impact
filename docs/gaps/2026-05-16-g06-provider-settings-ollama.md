...
Status: Batch/query semantic-edge, query-time provider rerank wiring, and full local temp-copy provider backfill verified as historical/candidate evidence; 2026-05-20 expanded-DB AQ09 live provider smoke remains blocked
...
- 2026-05-20 expanded default-DB acceptance is recorded in `docs/qa/2026-05-20-acceptance-data-asip-expanded.md` and remains a blocker, not a G06 closeout. It proves AQ09 separates persisted semantic-edge rows, provenance freshness, and live provider reachability: `14` `ollama/gemma4:e4b` `extractor=semantic_edges` rows from job `4` exist while `11` same-provider doc-node rows are ignored, but latest index job `10` is newer, so `semantic_edge_provenance` is `partial/stale`; live `semantic_edge` smoke fails with `Operation not permitted`. The same artifact has only `27` provider embeddings and `125962` deterministic fallbacks, so the embedding check proves provider provenance exists, not full current-DB provider-vector coverage.

## Remaining Gap

Saved embedding settings can drive the provider client in indexing, post-index backfill, and query-time provider-vector rerank. Failed live indexing/query embedding calls fall back to deterministic vectors with explicit metadata. Local Ollama semantic-edge model calls are proven historically against the clean AMD DB with `gemma4:e4b` provider smoke and against the live graph DB with persisted `gemma4:e4b` semantic/doc-node jobs. The current expanded `data/asip.db` gate is still open because AQ09 live semantic-edge provider smoke fails in this environment and credentialed live OpenAI-compatible QA is still open.
