...
Status: Current default-DB provider gate, local OpenAI-compatible live smoke, batch/query semantic-edge, query-time provider rerank wiring, and full provider embedding coverage are verified; user accepted local Ollama/gemma as the active OpenAI-compatible provider path
...
- 2026-05-18 OpenAI-compatible example hardening: `configs/edge_cases/full-corpus-openai-compatible-example.json` now uses `Bearer ${ENV:OPENAI_API_KEY}` instead of the non-expanded `${OPENAI_API_KEY}` form. A RED/GREEN regression in `packages.core.tests.test_semantic_edges.SemanticEdgeFeatureTests.test_committed_openai_compatible_example_config_uses_chat_completions` proves the committed example matches the supported late-bound secret syntax.
...
- 2026-05-20 expanded default-DB acceptance is recorded in `docs/qa/2026-05-20-acceptance-data-asip-expanded.md` as historical blocker evidence. It exposed stale semantic-edge provenance and partial provider-vector coverage in an older expanded DB artifact; later current artifacts supersede that state.
- 2026-05-21 current live artifacts now prove the default `data/asip.db` provider path: `docs/qa/2026-05-21-provider-gate-current.json` passes all five provider checks with `ollama/gemma4:e4b` semantic/doc-node provenance and `147841 / 147841` provider embeddings, while `docs/qa/2026-05-21-acceptance-data-asip-live-web-current.json` passes AQ01-AQ09 across CLI/API/API_LIVE/Web/MCP/MCP_PROTOCOL.
- 2026-05-21 OpenAI-compatible live smoke is recorded in `docs/qa/2026-05-21-openai-compatible-live-smoke.json` and `.md`. It uses Ollama's OpenAI-compatible `/v1/embeddings` and `/v1/chat/completions` endpoints as a real live compatibility surface: `nomic-embed-text:latest` returns a 768-dimensional embedding and `gemma4:e4b` returns one product-schema-persistable edge for `GCVM_L2_CNTL sets_field ENABLE_L2_CACHE`. This proves the compatible protocol path without code changes.
- 2026-05-21 completion-gate follow-up: the aggregate completion gate now takes `--hosted-openai-json` and requires the `asip.openai_compatible_live_smoke` artifact to pass both `/v1/embeddings` and `/v1/chat/completions` checks. It accepts either a `hosted-credentialed` artifact, or a local-compatible Ollama artifact only when G13 records explicit user acceptance of that local provider boundary.
- 2026-05-21 semantic rerank quality evaluation is recorded in `docs/qa/2026-05-21-semantic-rerank-quality-eval.json` and `.md`. It confirms full current-DB provider embedding coverage (`147841 / 147841` chunks), AQ01-AQ09 live acceptance across all product surfaces, and one current AQ query (`AQ05`) with explicit `provider-vector` participation. It is intentionally `partial` because it does not claim hosted production semantic ranking quality across arbitrary future corpora.
- 2026-05-21 labeled semantic quality evaluation is recorded in `docs/qa/2026-05-21-semantic-rerank-labeled-eval.json` and `.md`. It runs `docs/qa/semantic-rerank-eval-set.jsonl` against the current DB and passes `8/8` cases with two provider-vector cases, one graph-target case for the natural-language `CP_HQD_*` wildcard query, and MRR `0.7643`.

## Remaining Gap

Saved embedding settings can drive the provider client in indexing, post-index backfill, and query-time provider-vector rerank. Failed live indexing/query embedding calls fall back to deterministic vectors with explicit metadata. Local Ollama semantic-edge model calls are proven historically against the clean AMD DB with `gemma4:e4b` provider smoke and against the live graph DB with persisted `gemma4:e4b` semantic/doc-node jobs. The current expanded `data/asip.db` gate is closed for local Ollama provider reachability, semantic-edge/doc-node provenance, and full provider embedding coverage. OpenAI-compatible live protocol support is proven through the local Ollama `/v1` compatibility surface.

Local Ollama availability, a small job-level AQ09 provider path, isolated Web BFF AQ09 provenance, Settings UI AQ09 wiring, one real UI-to-BFF isolated-DB AQ09 run, callable semantic-edge product jobs, clean DB local Ollama AQ09 provider checks, and a local OpenAI-compatible live `/v1` smoke are now proven. The user explicitly selected this local Ollama/gemma path because no hosted `OPENAI_API_KEY` is available.

The batch semantic-edge product path is now implemented and tested for indexed candidates and graph refresh. Remaining provider risk is operational: local Ollama generation is slow/heavy on `gemma4:e4b`; hosted OpenAI-compatible provider QA is deferred unless credentials are supplied later.

The current default DB now has full local provider embedding coverage, and quality-evaluation artifacts confirm AQ01-AQ09 surface consistency, visible provider-vector participation, and a labeled current-corpus semantic-quality pass. Provider status must still not imply that a hosted OpenAI-compatible provider generated every current default-DB vector unless job metadata proves it for that DB. Semantic ranking quality at production scale still depends on future corpus distributions and model quality.

OpenAI-compatible smoke now validates request shape, safe secret/env header expansion, and a live local `/v1` compatible endpoint. Hosted credentialed OpenAI-compatible endpoint checks remain available through environment configuration, but they are not the active completion path without a supplied credential.
