...
- Clean AMD verification now exists at `/tmp/asip-clean-amd-qwen35-provider-2026-05-17.db` with `documents=124`, `chunks=21884`, `evidence=860543`, `edges=23`, and `embeddings=961`. Document source counts are `code=7`, `doc=20`, `pdf=1`, `register=96`; evidence source counts are `code=153`, `doc=5664`, `pdf=5`, `register=854721`.
- Final current acceptance artifact `docs/qa/2026-05-17-acceptance-clean-amd-qwen35-provider-current.json` and `.md` records AQ01-AQ09 as `9 passed, 0 partial, 0 failed` against that clean AMD DB with DB health pass.
