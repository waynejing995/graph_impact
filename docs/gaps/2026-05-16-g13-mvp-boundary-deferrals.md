...
- 2026-05-18 follow-up narrowed several previous residuals: G07 now has deterministic structured resolved-chain explanations, Web/MCP graph parity, and bundled-Python real MCP runtime smoke; G15 now has repeat deterministic graph rebuild timing over live AMD DB backups; G06 now has full local temp-copy Ollama provider embedding coverage timing. These do not close full clangd/libclang cross-TU vtable/type-flow, credentialed OpenAI-compatible live QA, query-time provider reranking, scanned-PDF OCR, or full raw re-index timing.
...
| Embedding provider and optional semantic-edge provider via Ollama/OpenAI-compatible APIs | MVP-1 / partial | G06 tracks split provider settings, safe env-based extra headers, embedding calls/backfill, semantic-edge jobs, full local temp-copy provider backfill timing, and live `gemma4:e4b`/`nomic-embed-text:latest` model QA. | Required by user for embeddings; semantic-edge model support requested. | Local Ollama path, safe header expansion, and full local temp-copy provider-vector coverage are proven; query-time rerank and credentialed OpenAI-compatible live QA remain residual boundaries needing acceptance, credentials, or implementation. |
...
| Full raw AMD re-index timing | Partial / operational boundary | G15 now has fixture rebuild, repeat deterministic graph rebuild timing, more than five query timings, and full local temp-copy provider embedding backfill timing. | User asked for real graph generation and tests; full empty-DB raw re-index timing has not been explicitly accepted as a blocking runtime requirement. | Full raw re-index remains a long-running QA item unless explicitly required for MVP closure. |
