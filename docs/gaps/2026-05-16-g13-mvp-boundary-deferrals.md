...
| Embedding provider and optional semantic-edge provider via Ollama/OpenAI-compatible APIs | MVP-1 / partial | G06 tracks split provider settings, embedding calls/backfill, semantic-edge jobs, and live `gemma4:e4b`/`nomic-embed-text:latest` model QA. | Required by user for embeddings; semantic-edge model support requested. | Local Ollama path is proven; credentialed OpenAI-compatible live QA remains a residual boundary needing acceptance or credentials. |
