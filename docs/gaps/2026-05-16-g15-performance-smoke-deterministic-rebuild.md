...
- 2026-05-18 edge-count summary follow-up is recorded in `docs/qa/2026-05-18-g15-edge-count-summary-qa.md`. Index summaries now count only persisted edge ids after `AsipStore.add_edge()` rejects resolver wrapper or macro hubs; the CLI fixture test asserts `index_payload["edges"] == select count(*) from edges`.
