...
- 2026-05-17 graph visual correction after user review: default graph budgeting now preserves the call/callback backbone, the force graph has a larger 2K viewport, tuned forces, and initial `zoomToFit`, and the relationship panel now previews `calls` edges first. Browser artifacts are recorded in `docs/qa/browser/graph-backbone-budget-after-fit-snapshot.md` and `docs/qa/browser/graph-backbone-budget-after-fit-2k.png`.
