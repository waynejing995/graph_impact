...
- Graph entity and edge normalization is a core/storage capability. The default product graph node contract allows only `function`, `register`, and `doc`; legacy document subtypes such as `doc_section`, `pdf_section`, and `doc_box` may exist only in raw/debug views or as `doc.attr.doc_kind`. Resolver macros, macro-expansion helpers, field-only symbols, raw source paths, temporary variables, and provider/config names stay in provenance or BoxMatrix-style node attributes. Product edge relations are normalized to a small enum before Web/API rendering.
...
- 2026-05-18 local receiver alias follow-up: core now owns two bounded receiver-alias paths that were previously too broad under receiver-type fallback. Device-level callback table fields such as `adev->userq_funcs[...] = &userq_mes_funcs` can flow into local receivers such as `uq_funcs->map(...)` with `type_flow=source_receiver_table_alias`, and local struct aliases such as `ip_block = &adev->ip_blocks[i]` can preserve registered `ip_block->version->funcs` paths with `type_flow=local_receiver_path_alias`. Tests also prove function-local aliases do not leak into other functions. A follow-up review fix prevents dynamic indexed paths from collapsing multiple known slots into the first table: multi-candidate userq and IP-block paths now stay `vtable_dispatch` with `*_ambiguous` type-flow provenance, even when only one candidate table implements the requested slot. This is still source-level aliasing, not full clangd/libclang points-to.
- 2026-05-18/19 cross-repo register identity follow-up: graph identity for registers now belongs to core/storage and defaults to canonical symbol plus IP: `register:{ip}:{symbol}`. IP versions are preserved in `attr.ip_versions` and per-source provenance rather than splitting the node, so `mxgpu` and `linux-amdgpu` functions can connect through the same register node when the IP and symbol match. Resolver profiles can override the product identity through `graph.register_normalization.identity`, and disabled DB aliases disable the loaded YAML profile id too.
...
- Stage 1 code graph pipeline: clang/preprocessor or compile-context strategy, selective clang AST JSON receiver and callback-initializer hints, direct helper call extraction, C ops/vtable callback slot extraction, bounded local/device receiver aliasing, generic dispatch-candidate expansion, fallback/error behavior, graph schema fields, node/edge kinds, deterministic provenance, tests, and Web rendering expectations. Full clangd/libclang cursor/type-flow callback resolution remains separate future ownership and must not be claimed by the current conservative extractor.
...
- Core normalizes product graph nodes to `function`, `register`, and `doc`, keeps legacy document subtype names in `attr.doc_kind`, and normalizes product graph relations to the documented enum, with macro/field/source/provider wording carried in BoxMatrix-style attributes instead of rendered as nodes or relation drift.
