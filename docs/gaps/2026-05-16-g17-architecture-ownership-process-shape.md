...
- 2026-05-18 cross-repo register identity follow-up: graph identity for registers now belongs to core/storage and uses canonical symbol, IP, and IP version only. Source/corpus/repo/path records merge into node attrs, so `mxgpu` and `linux-amdgpu` functions can connect through the same register node. Known different IP versions remain separate nodes.
