...
| G09 | [SQLite FTS5 Vector And NetworkX Runtime](2026-05-16-g09-storage-vector-graph-runtime.md) | Partial; runtime-boundary open | Native sqlite-vec extension smoke is verified in the bundled Python runtime, system Python keeps the documented fallback/skip path, and NetworkX graph runtime is proven in final QA; product retrieval still uses JSON vectors plus Python cosine until a native retrieval adapter is wired. |
...
- P1: G09 native sqlite-vec extension smoke is verified in the bundled Python runtime, but product retrieval still uses the fallback adapter; native sqlite-vec retrieval acceleration remains unwired.
