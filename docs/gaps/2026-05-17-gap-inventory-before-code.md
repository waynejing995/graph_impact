...
| G06 | [Provider Settings And Ollama Detection](2026-05-16-g06-provider-settings-ollama.md) | Partial; blocking | Local Ollama settings now drive embeddings and query/batch semantic-edge jobs with provenance. The clean AMD provider artifact now uses `gemma4:e4b` instead of stale qwen rows; remaining proof is OpenAI-compatible credential/secret handling or explicit local-only deferral, provider failure evidence, and query-time rerank boundary. |
