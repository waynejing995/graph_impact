...
- Partial or explicitly deferred boundaries: G02, G06, G09, G13, G15, AQ, plus the explicit G03 full clangd/libclang residual. G07 real MCP runtime smoke is now verified in the bundled Python 3.12 runtime, and G06 now has full local temp-copy provider embedding coverage.
...
- 2026-05-18 vtable correction: full clangd/libclang vtable type-flow parsing is not done. Current callback/common links come from source-span table initializer and slot-call matching plus shallow receiver type hints. The typed callback precision slice adds selective `clang -Xclang -ast-dump=json` receiver-type provenance (`type_flow=clang_ast_json`) and now makes that typed receiver override generic `funcs` leaf-name fallback; it also resolves callback table initializers that point to uniquely known functions defined in another file. The AST JSON callback-initializer slice adds `callback_initializer_flow=clang_ast_json` for macro-wrapped initializers and records `6` such rows after the latest live rebuild. Latest default graph rebuild records `2,248` typed callback rows, `44` cross-file callback rows, and `21` exact table-alias callback rows, but `source=clang_callback` still must not be read as full clangd output. The next proof must either add a true typed clangd/libclang cross-TU extractor or keep this as an explicit residual, while query graphs show `common -> callback -> register` backbones and cross-repo graph connectivity through shared normalized register nodes.
...
| --- | --- | --- | --- |
| G01 | [Real Ingestion And Indexing](2026-05-16-g01-real-ingestion-indexing.md) | Current clean-final pass verified; final gate | The named clean-final artifact `/tmp/asip-clean-amd-gemma4-final-current-2026-05-18.db` records code/doc/pdf/register counts, successful clean jobs, browser/product-surface proof, and AQ01-AQ09 9/9. Default `data/asip.db` was later intentionally rebuilt for G03 graph QA, so it is not the G01 byte-identity proof; G11 still owns final residual review, commit, and push. |
...
| G03 | [Dynamic Weighted Graph](2026-05-16-g03-dynamic-weighted-graph.md) | Partial; blocking | Fresh real AMD rebuild and 2K browser QA now prove a real Stage 1 deterministic graph plus small Stage 2 `gemma4:e4b` overlay. This pass adds shared graph-budget config, compile_commands-driven macro preprocessing, conservative direct/callback callgraph edges, cross-file unique direct common-helper calls, cross-file callback initializer calls, generic common `vtable_dispatch` candidate edges, receiver type-hint filtering for `struct <type> *ops` and `(*ops->slot)(...)`, local/field-path callback table aliases, typed receiver precedence over generic `funcs` fallback, IP-block version/function-table aliases, corpus-level IP-block registration aliases, selective Clang AST JSON receiver-type hints, selective Clang AST JSON macro-wrapped callback-initializer hints, MxGPU `amdgv_init_func` dispatch coverage, query-scoped multi-seed performance fixes, exact function-node graph fallback, config-driven resolver-operator mega-node rejection at storage/import boundaries, Markdown/PDF section endpoint proof, and BoxMatrix-style LLM doc boxes. Latest QA records 10 real queries, a 3,000-edge global graph with 2,805 nodes, and in-app browser graph screenshots. Remaining proof is full Linux-kernel compile DB/clangd-quality cross-TU type-flow coverage, larger robust semantic batches, ranking review, and design-doc closure. |
...
- P1: G15 now has fixture stable-count rebuild, sub-second query evidence, repeat real-corpus deterministic graph rebuild timing, bounded provider backfill timing, and full local temp-copy provider coverage timing; it still needs full raw re-index timing if kept in scope.
