...
Current clean AMD run: `docs/qa/2026-05-17-acceptance-clean-amd-gemma4-provider-current.json` and `.md` were generated from `/tmp/asip-clean-amd-gemma4-provider-2026-05-17-final.db`. Summary: 9 total, 9 pass, 0 partial, 0 failed. DB health passes. Source counts are recorded in G01/G10. AQ05 passes with `code`, `doc`, `pdf`, and `register`; AQ06 passes with `code`, `doc`, and `register`; AQ09 provider checks pass with `ollama/nomic-embed-text:latest` embeddings and `ollama/gemma4:e4b` semantic-edge smoke. This is the current query-level acceptance artifact, while browser screenshots, full-suite reruns, and design review remain G10/G11 blockers.
...
| AQ02 | Which fields of `GCVM_L2_CNTL` are set in MxGPU `gfx_v11_0.c`? | G01, G02, G05, G10 | CLI, API, Web | Current clean AMD run: pass; source types `code/doc/register`. | Link final G10/G11 review and browser evidence. |
| AQ03 | Where is `IH_RB_CNTL` configured, and which fields are modified? | G01, G02, G03, G10 | CLI, API, Web | Current clean AMD run: pass; source types `code/doc/register`. | Link final G10/G11 review and browser evidence. |
...
| AQ08 | Add a toy Python resolver profile extracting a configured function-call or string-symbol reference, proving profiles are not macro-only. | G05, G07, G10, G13 | CLI, API, Web | Current clean AMD run: pass; source types `doc/register`. | Python profile workflow/browser proof remains G05/G13 evidence. |
| AQ09 | Run embedding and optional semantic-edge extraction through a configured Ollama provider, then switch to an OpenAI-compatible provider without changing retrieval or resolver code. | G06, G09, G10, G17 | CLI, API, Web, MCP | Current clean AMD run: pass for local Ollama `gemma4:e4b`/`nomic-embed-text:latest` provider checks. Credentialed OpenAI-compatible live endpoint remains an explicit boundary unless the user supplies credentials or accepts local-compatible evidence. | OpenAI-compatible credential path remains G06/G13 boundary; browser/settings proof remains G10/G11. |
