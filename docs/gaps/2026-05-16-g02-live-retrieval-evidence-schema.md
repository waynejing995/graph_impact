...
- `docs/qa/2026-05-21-semantic-rerank-labeled-eval.json` and `.md` record a repeatable labeled current-corpus semantic-quality gate. The eval set `docs/qa/semantic-rerank-eval-set.jsonl` covers five queries spanning provider-vector document/source-tree retrieval, exact register lookup, SDMA code-path retrieval, and macro-chain retrieval. Current result: `5/5` passed, two provider-vector cases, MRR `0.67`.
