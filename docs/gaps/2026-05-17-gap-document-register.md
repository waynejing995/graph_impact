...
| G03 | [Dynamic Weighted Graph](2026-05-16-g03-dynamic-weighted-graph.md) | Obsidian-style global graph and query-scoped graph rendered from weighted stored relationships. | Current pass verified. NetworkX-backed graph data includes persisted/evidence edges, function operation edges, conservative direct/callback call edges, document section nodes, and batch semantic edges; `/graph` uses `react-force-graph-2d`. | Preserve current QA evidence through final G11 review; rerun visual tests if UI changes again. |
...
| G09 | [SQLite FTS5 Vector And NetworkX Runtime](2026-05-16-g09-storage-vector-graph-runtime.md) | SQLite owns storage, FTS5/vector retrieval works, and NetworkX is the graph runtime. | Partial. FTS5/fallback vector/NetworkX are live, and native sqlite-vec extension smoke is verified in the bundled Python runtime; product retrieval still uses JSON vectors plus Python cosine until a native retrieval adapter is wired. | Final query/graph artifacts report retrieval sources and `graph_runtime: networkx`; native retrieval acceleration and provider-vector/rerank quality remain separate boundaries. |
...
| "Corpus cannot really be added/indexed from the UI." | G04, G07, G10 | Real Web add-index-query loop is proven; clean-DB repeat and durable job-state UX remain G04 residuals. |
