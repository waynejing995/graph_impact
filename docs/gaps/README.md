...
acceptance/browser/provider/Stage 2/callback/git gates pass, and the user
accepted local Ollama/gemma as the OpenAI-compatible provider path for the
active goal
...
is still not complete until the latest audit changes are committed, pushed, and
verified through the G11 post-push aggregate gate.
...
| G06 | [Provider Settings And Ollama Detection](2026-05-16-g06-provider-settings-ollama.md) | Current local/batch/full-temp-backfill/query-rerank pass verified; local Ollama/gemma provider boundary accepted | Settings persist/hydrate, embedding provider calls, safe env-based extra-header expansion, isolated AQ09 Web API provenance, Settings AQ09 UI/BFF wiring, query-scoped and batch semantic-edge jobs, query-time provider-vector rerank wiring, current clean local Ollama `gemma4:e4b`/`nomic` QA, historical `qwen3.5` comparison artifacts, bounded 128-chunk provider backfill, full local temp-copy provider embedding coverage (`21884 / 21884`, missing 0), and local OpenAI-compatible `/v1` smoke are tested; hosted OpenAI-compatible QA is deferred because the user selected Ollama/gemma for this active goal. |
...
| G10 | [Testing Acceptance And Visual QA](2026-05-16-g10-testing-acceptance-visual-qa.md) | Current expanded-DB acceptance and browser QA pass | Clean-final and 2026-05-19 browser artifacts remain candidate/historical. The current 2026-05-21 `data/asip.db` artifacts pass AQ01-AQ09 across CLI/API/API_LIVE/Web/MCP/MCP_PROTOCOL, pass the required no-mock browser e2e subset `5/5`, pass Web no-server smoke `9/9`, discover `109` Playwright tests for route/API/workbench coverage, and include concept-detail browser proof for selecting a real `:concept:` function node and viewing `Concept Generated From` implementation details. |
| G11 | [Completion Gate And Documentation Review](2026-05-16-g11-completion-gate.md) | Final gate refresh in progress | The 2026-05-21 post-push aggregate gate is the authoritative closure path. Browser e2e is regenerated live out of tree and bound to the git head; no-server smoke binds fresh artifact paths and SHA; residual acceptance binds the current G13 document SHA. The gate now supports the user-accepted local Ollama/gemma OpenAI-compatible provider path, while hosted credentialed QA remains optional future evidence if credentials are supplied. |
...
| G13 | [MVP Boundary And Full-Spec Deferrals](2026-05-16-g13-mvp-boundary-deferrals.md) | Accepted residuals recorded | Long-range full-spec items are explicitly deferred, and the user accepted the remaining active-goal residual boundaries including local Ollama/gemma as the OpenAI-compatible provider path. |
...
| G6. Configurable resolvers and providers | G05, G06, AQ07-AQ09 | Current local provider/provenance pass with accepted local Ollama/gemma boundary | UI/API workflow that changes resolver/provider behavior without code edits; hosted credentialed OpenAI-compatible QA remains optional future evidence when credentials exist. |
...
| Full-spec future capabilities and deferrals | G13 | Deferral ledger exists; user acceptance recorded | Deferral ledger mapping full-spec items to MVP/deferred/user-accepted status. |
