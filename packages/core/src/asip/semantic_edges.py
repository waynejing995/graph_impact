    ...
    return [
        {
            ...
            "content": (
                ...
                "---\n"
                "FIRST, classify the document TEXT into ONE type:\n"
                "- CODE_SNIPPET: RREG32/WREG32/REG_SET_FIELD function calls\n"
                "- C_ENUM: enum Foo { A, B, C } or Field|Description table of enum members\n"
                "- API_DOC: function with Description / Input parameters / Output sections\n"
                "- EVENT_CATEGORY: Category|Description table listing event types (NOT enum values)\n"
                "- REGISTER_DEF: #define NAME value with address or bitfield comment\n"
                "- METRIC_ENUM: Field|Description table of telemetry counter names\n"
                "- WORKFLOW: numbered steps, state machine, or procedure\n"
                "---\n"
                "THEN extract each attribute by type:\n\n"
                "name (3-8 word title):\n"
                "  CODE_SNIPPET: '{func} register access'\n"
                "  C_ENUM: '{EnumType} enum values'\n"
                "  API_DOC: '{func} API'\n"
                "  EVENT_CATEGORY: '{Category} event category'\n"
                "  REGISTER_DEF: '{REG} register'\n"
                "  METRIC_ENUM: '{Group} metric group'\n"
                "  WORKFLOW: '{name} workflow'\n\n"
                "summary:\n"
                "  1-3 sentences from the nearest descriptive sentence, paragraph header, or table caption. "
                "Verbatim preferred. Do NOT extrapolate causal relationships not stated in the source.\n\n"
                "inputs (by type):\n"
                "  CODE_SNIPPET = registers read (RREG32 targets / RREG32 calls)\n"
                "  API_DOC = each 'Input parameter:' list entry with type+description\n"
                "  C_ENUM = [] unless the text describes trigger conditions for individual enum values\n"
                "  EVENT_CATEGORY = trigger condition IF the text describes what causes the event, else []\n"
                "  REGISTER_DEF = field values or write data if documented, else []\n"
                "  METRIC_ENUM = [] (metrics are read-only measurements, not triggered)\n"
                "  WORKFLOW = entry condition or prerequisite\n\n"
                "outputs (by type):\n"
                "  CODE_SNIPPET = registers written (WREG32 targets / WREG32 calls)\n"
                "  C_ENUM = EACH enum member as a separate string entry (e.g. ['RESET_GPU','RESET_FLR'])\n"
                "  API_DOC = each 'Output:' field with its description\n"
                "  EVENT_CATEGORY = [] (a category table is a grouping mechanism, not a producer)\n"
                "  REGISTER_DEF = read-back values or field semantics if described, else []\n"
                "  METRIC_ENUM = each metric field name with its description\n"
                "  WORKFLOW = completion state or result data\n\n"
                "constraints:\n"
                "  Extract only explicit language: 'must', 'shall', 'should', 'only if', 'Note:', 'cannot'. "
                "Do NOT infer latent constraints. Default [].\n\n"
                "evidence:\n"
                "  Verbatim source line(s) proving the box. ≤140 chars. Include source line number when available.\n\n"
                "confidence:\n"
                "  0.95 = explicit definition/labeled table\n"
                "  0.85 = clear but implicit boundary\n"
                "  0.70 = inferred from context\n"
                "  0.55 = best-guess\n\n"
                "Relationships:\n"
                "  src/dst = box id (from same document) or LINKED SYMBOL (function or register identifier). "
                "Register fields (FIELD_NAME_MASK) go in inputs/outputs, NOT as endpoints. "
                "Prefer LINKED SYMBOLS from the TEXT or 'LINKED SYMBOLS' list; do not invent symbols. "
                "Relation options: reads, writes, sets_field, calls, reports_on, depends_on, triggers."
            ),
        },
        {"role": "user", "content": f"/no_think\n{prompt}"},
    ]
